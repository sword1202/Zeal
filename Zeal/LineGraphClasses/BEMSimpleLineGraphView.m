...
#define XLABEL_WIDTH 60
#define XLABEL_HEIGHT 20
...
- (void)drawGraph {
    if (self.isLastMonthTransactions) {
        // Draw the graph
        [self drawDots];
    } else
    {
...
- (void)drawEntireGraph {
    ...
    self.maxValue = self.app.maxAmount;
...
#pragma clang diagnostic pop
            ...
            {
                if (self.app.isMaxInLast) {
                    // get current month transactions
                    dotValue = [[self.app.arrForLineGraphDataCurrentMonth objectAtIndex: i] floatValue];
                } else
                {
                    // get last month transactions
                    dotValue = [[self.app.arrForLineGraphDataLastMonth objectAtIndex: i] floatValue];
                }
            ...
            if (dotValue != BEMNullGraphValue) {
                BEMCircle *circleDot = [[BEMCircle alloc] initWithFrame:CGRectMake(0, 0, self.sizePoint, self.sizePoint) monthFlag:0];// greencolor for last month
                
                if (self.app.isMaxInLast) {
                    circleDot = [[BEMCircle alloc] initWithFrame:CGRectMake(0, 0, self.sizePoint, self.sizePoint) monthFlag:1];// bluecolor for current month
                    if (!self.isLastMonthTransactions) {
                        circleDot = [[BEMCircle alloc] initWithFrame:CGRectMake(0, 0, self.sizePoint, self.sizePoint) monthFlag:0];// bluecolor for last month
                ...
                if (self.animationGraphEntranceTime == 0) {
                    if (self.displayDotsOnly == YES) circleDot.alpha = 1.0;
                        ...
                        if (self.alwaysDisplayDots == NO) circleDot.alpha = 1.0;
...
- (void)drawLine {
    ...
    BEMLine *line = [[BEMLine alloc] initWithFrame:[self drawableGraphArea] monthFlag: 0]; // last month
    
    if (self.app.isMaxInLast) {
        line = [[BEMLine alloc] initWithFrame:[self drawableGraphArea] monthFlag: 1]; // current month
        if (!self.isLastMonthTransactions) {
//            for (UIView *subview in [self subviews]) {
//                if ([subview isKindOfClass:[BEMLine class]])
//                    [subview removeFromSuperview];
//            }
            line = [[BEMLine alloc] initWithFrame:[self drawableGraphArea] monthFlag: 0]; // last month
...
- (void)drawXAxis {
    ...
    self.backgroundXAxis.backgroundColor = [UIColor redColor];
...
#pragma clang diagnostic pop
        ...
        if (numberOfGaps >= (numberOfPoints - 1)) {
            ...
            NSString *secondXLabel = [self xAxisTextForIndex:1];
            NSString *thirdXLabel = [self xAxisTextForIndex:2];
            NSString *fourthXLabel = [self xAxisTextForIndex:3];
            NSString *lastXLabel = [self xAxisTextForIndex:4];
            
            CGFloat viewWidth = self.frame.size.width - self.YAxisLabelXOffset;
            
            CGFloat xAxisXPositionFirstOffset, xAxisXPositionSecondOffset, xAxisXPositionThirdOffset, xAxisXPositionFourthOffset;
            ...
            if (self.positionYAxisRight) {
                ...
                xAxisXPositionFirstOffset = 5;
                xAxisXPositionSecondOffset = xAxisXPositionFirstOffset + viewWidth/5;
                xAxisXPositionThirdOffset = xAxisXPositionSecondOffset + viewWidth/5;
                xAxisXPositionFourthOffset = xAxisXPositionThirdOffset + viewWidth/5;
                xAxisXPositionLastOffset = xAxisXPositionFourthOffset + viewWidth/5;
            }
            UILabel *firstLabel = [self xAxisLabelWithText:firstXLabel atIndex:0];
            firstLabel.frame = CGRectMake(xAxisXPositionFirstOffset, self.frame.size.height-XLABEL_HEIGHT, XLABEL_WIDTH, XLABEL_HEIGHT);
            firstLabel.textAlignment = NSTextAlignmentCenter;
            [self addSubview:firstLabel];
            [xAxisValues addObject:firstXLabel];
            [xAxisLabels addObject:firstLabel];
            
            UILabel *secondLabel = [self xAxisLabelWithText:secondXLabel atIndex:1];
            secondLabel.frame = CGRectMake(xAxisXPositionSecondOffset, self.frame.size.height-XLABEL_HEIGHT, XLABEL_WIDTH, XLABEL_HEIGHT);
            secondLabel.textAlignment = NSTextAlignmentCenter;
            [self addSubview:secondLabel];
            [xAxisValues addObject:secondXLabel];
            [xAxisLabels addObject:secondLabel];
            
            UILabel *thirdLabel = [self xAxisLabelWithText:thirdXLabel atIndex:2];
            thirdLabel.frame = CGRectMake(xAxisXPositionThirdOffset, self.frame.size.height-XLABEL_HEIGHT, XLABEL_WIDTH, XLABEL_HEIGHT);
            thirdLabel.textAlignment = NSTextAlignmentCenter;
            [self addSubview:thirdLabel];
            [xAxisValues addObject:thirdXLabel];
            [xAxisLabels addObject:thirdLabel];
            
            UILabel *fourthLabel = [self xAxisLabelWithText:fourthXLabel atIndex:3];
            fourthLabel.frame = CGRectMake(xAxisXPositionFourthOffset, self.frame.size.height-XLABEL_HEIGHT, XLABEL_WIDTH, XLABEL_HEIGHT);
            fourthLabel.textAlignment = NSTextAlignmentCenter;
            [self addSubview:fourthLabel];
            [xAxisValues addObject:fourthXLabel];
            [xAxisLabels addObject:fourthLabel];
            
            UILabel *lastLabel = [self xAxisLabelWithText:lastXLabel atIndex:4];
            lastLabel.frame = CGRectMake(xAxisXPositionLastOffset, self.frame.size.height-XLABEL_HEIGHT, XLABEL_WIDTH, XLABEL_HEIGHT);
            ...
            if (self.positionYAxisRight) {
                ...
                NSNumber *xSecondAxisLabelCoordinate = @(secondLabel.center.x - self.YAxisLabelXOffset);
                NSNumber *xThirdAxisLabelCoordinate = @(thirdLabel.center.x - self.YAxisLabelXOffset);
                NSNumber *FourthAxisLabelCoordinate = @(fourthLabel.center.x - self.YAxisLabelXOffset);
                NSNumber *xLastAxisLabelCoordinate = @(lastLabel.center.x - self.YAxisLabelXOffset);
//                [xAxisLabelPoints addObject:xFirstAxisLabelCoordinate];
                [xAxisLabelPoints addObject:xSecondAxisLabelCoordinate];
                [xAxisLabelPoints addObject:xThirdAxisLabelCoordinate];
                [xAxisLabelPoints addObject:FourthAxisLabelCoordinate];
                [xAxisLabelPoints addObject:xLastAxisLabelCoordinate];
            }
        } else {
            @autoreleasepool {
...
- (UILabel *)xAxisLabelWithText:(NSString *)text atIndex:(NSInteger)index {
    ...
    labelXAxis.baselineAdjustment = UIBaselineAdjustmentAlignBaselines;
    labelXAxis.adjustsFontSizeToFitWidth = YES;
    labelXAxis.adjustsFontForContentSizeCategory = YES;
    labelXAxis.clipsToBounds = YES;
    CGPoint center;
...
//    } else horizontalTranslation = 0;
    ...
    if (index == numberOfPoints - 1) {
//        rect = CGRectMake(self.frame.size.width - 40, self.frame.size.height-XLABEL_HEIGHT, 40, XLABEL_HEIGHT);
//        lRect = [labelXAxis.text boundingRectWithSize:rect.size options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:labelXAxis.font} context:nil];
//        labelXAxis.frame = rect;
//        labelXAxis.center = CGPointMake(self.frame.size.width, self.frame.size.height - lRect.size.height/2);
    }
    return labelXAxis;
}
...
- (void)drawYAxis {
    ...
    if (self.positionYAxisRight) {
        ...
        frameForBackgroundYAxis = CGRectMake(0, 0, self.YAxisLabelXOffset-10, self.frame.size.height-XLABEL_HEIGHT);
        frameForLabelYAxis = CGRectMake(-10, 0, self.YAxisLabelXOffset - 5, 15);
...
//  CGRectMake(xAxisXPositionFirstOffset, self.frame.size.height-20, viewWidth/2, 20);
    NSInteger xAxisHeight = XLABEL_HEIGHT;
...
- (void)reloadGraph {
    ...
    self.isLastMonthTransactions = NO;
    self.app = (AppDelegate *) [UIApplication sharedApplication].delegate;
    NSLog(@"GetData---:\n%@", self.app.arrForLineGraphDataLastMonth);
    if (self.app.isMaxInLast) {
        if (self.app.arrForLineGraphDataCurrentMonth && self.app.arrForLineGraphDataCurrentMonth.count > 1) {
            self.isLastMonthTransactions = YES;
    ...
    {
        if (self.app.arrForLineGraphDataLastMonth && self.app.arrForLineGraphDataLastMonth.count > 1) {
            self.isLastMonthTransactions = YES;
